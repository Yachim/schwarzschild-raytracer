...
#define MAX_CURVE_POINTS 300
struct Curve {
    vec3 points[MAX_CURVE_POINTS];
    int num_points;
    float radius;
};

#define MAX_CURVES 10
uniform Curve curves[MAX_CURVES];
...
vec4 calculate_lighting(HitInfo hit_info, vec3 view_dir) {
    ...
    if (hit_info.object.type == OBJECT_TYPE_CURVE) return material.color;
...
HitInfo curve_intersect(Ray ray, Curve curve, float max_lambda) {
    HitInfo closest_hit;
    closest_hit.is_hit = false;
    if (curve.num_points < 2) return closest_hit;

    for (int i = 0; i < curve.num_points - 1; i++) {
        vec3 diff = curve.points[i + 1] - curve.points[i];

        HitInfo hit = cylinder_intersect(ray, Cylinder(
            Transform(
                curve.points[i] + diff / 2,
        ...
        if (!closest_hit.is_hit || hit.dist < closest_hit.dist) closest_hit = hit;
    }
...
HitInfo intersect_object(Ray ray, Object object, float max_lambda) {
    ...
    switch(object_type) {
        ...
        case OBJECT_TYPE_CURVE:
            Curve curve = curves[object_index];
            res = curve_intersect(ray, curve, max_lambda);
            break;
    }
    res.object = object;
